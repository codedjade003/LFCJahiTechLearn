...
const ProfilePage = () => {
  ...
  useEffect(() => {
    if (user?.profilePicture?.position && 
        'x' in user.profilePicture.position && 
        'y' in user.profilePicture.position) {
      const position = user.profilePicture.position as unknown as { x: number; y: number };
      setProfilePicturePosition(position);
    }
