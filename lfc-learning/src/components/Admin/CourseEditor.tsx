...
export default function CourseEditor({ courseId, onBack, onCourseUpdated }: CourseEditorProps) {
    ...
    if (loading) {
      return (
        <div className="animate-pulse space-y-4">
          <div className="h-8 bg-gray-200 dark:bg-[var(--bg-tertiary)] rounded"></div>
          <div className="h-32 bg-gray-200 dark:bg-[var(--bg-tertiary)] rounded"></div>
          <div className="h-8 bg-gray-200 dark:bg-[var(--bg-tertiary)] rounded"></div>
        </div>
      );
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-screen bg-yt-light-gray dark:bg-[var(--bg-primary)] text-yt-text-dark dark:text-[var(--text-primary)]">
        <header className="sticky top-0 z-40 flex items-center justify-between px-4 py-3 border-b border-[var(--border-primary)] bg-white dark:bg-[var(--bg-elevated)] shadow-sm">
          <div className="flex items-center">
            <button 
              onClick={onBack}
              className="mr-3 p-2 rounded-md text-yt-text-dark dark:text-[var(--text-primary)] hover:bg-gray-200 dark:bg-[var(--bg-tertiary)] dark:hover:bg-[var(--hover-bg)]"
            >
              <FaArrowLeft />
            </button>
            <button 
              className="md:hidden mr-3 p-2 rounded-md text-yt-text-dark dark:text-[var(--text-primary)] hover:bg-gray-200 dark:bg-[var(--bg-tertiary)] dark:hover:bg-[var(--hover-bg)]"
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            >
              <FaBars />
            </button>
            <div className="p-2 rounded-full bg-lfc-red dark:bg-red-800 text-gray-200 mr-3">
        ...
        {isSidebarOpen && (
          ...
          <main className="flex-1 bg-yt-light-bg dark:bg-[var(--bg-primary)] p-3 md:p-6">
    ...
    <div className="flex flex-col h-screen bg-yt-light-gray dark:bg-[var(--bg-primary)] text-yt-text-dark dark:text-[var(--text-primary)]">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto bg-yt-light-bg dark:bg-[var(--bg-primary)] p-3 md:p-6">
          <div className="max-w-4xl mx-auto">
            {/* Progress indicator */}
            <div className="mb-6 bg-white dark:bg-[var(--bg-elevated)] p-4 md:p-5 rounded-lg shadow-sm border border-[var(--border-primary)]">
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs md:text-sm text-[var(--text-secondary)]">Step {activeIndex + 1} of {tabs.length}</span>
                <span className="text-xs md:text-sm font-medium text-lfc-gold">{tabs[activeIndex].label}</span>
              </div>
              <div className="relative w-full h-2 bg-[var(--bg-tertiary)] rounded-full">
                <div
            ...
            {/* Active Tab Content */}
            <div className="bg-white dark:bg-[var(--bg-elevated)] rounded-lg p-4 md:p-6 border border-[var(--border-primary)] shadow-sm">
              {renderTabContent()}
            </div>
              ...
              <button
                ...
                className={`px-4 py-2 md:px-6 md:py-2.5 rounded-md font-medium transition-colors text-sm ${
                  activeIndex === tabs.length - 1 
                    ? "bg-lfc-red dark:bg-red-800 hover:bg-lfc-red-hover dark:hover:bg-red-700 text-gray-200"
                    : "bg-lfc-red dark:bg-red-800 hover:bg-lfc-red-hover dark:hover:bg-red-700 text-gray-200"
                }`}
              >
