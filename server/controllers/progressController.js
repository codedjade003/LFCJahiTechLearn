...
export const calculateCourseProgress = (enrollment, course) => {
  ...
  // Ensure progress never exceeds 100% and is never negative
  const rawProgress = totalItems > 0 ? (completedItems / totalItems) * 100 : 0;
...
export const updateEnrollmentProgress = async (enrollmentId, courseId) => {
  try {
    ...
    // Calculate total progress - ensure it never exceeds 100% or goes below 0%
    const rawProgress = (moduleProgress + assignmentProgress + projectProgress) * 100;
